...
var CSVParser = {
  ...
  isNumber: function (string) {
    if ((string == null) || isNaN(parseInt(string))) {
      return false;
    }
    return true;
  },
...
const sqlHeader = `
  ...
  select *
  FROM [ICCGroupType] lit   

  select *
  FROM [ICCPaymentType] lit

  select *
  FROM [ICCFee] li
  where DiscontinueDate like '%2999%'

  insert into [ICCFee](iccGroupCodeKey, iccPaymentTypeKey, iccFee, DiscontinueDate, EffectiveDate)
  values
`;

export const format = (discontinueDate, effectiveDate, data) => {
  return new Promise((resolve, reject) => {
    ...
    const dataArray = CSVParser.parse(data, false, ',', false, false).dataGrid;
    ...
    resolve(sqlHeader + output);
  });
};

export const getDownload = (effectiveDate, output) => {
  const month = new Date(effectiveDate).toDateString().split(' ')[1];
  const year = new Date(effectiveDate).toDateString().split(' ')[3];

  download(`ICC${month}${year}Scripts.sql`, output);
};
